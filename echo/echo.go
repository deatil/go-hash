...
func (d *digest) checkSum() []byte {
    ...
    if d.nx > limit - 18 {
        copy(d.x[d.nx:], zeros[:limit - d.nx])
        d.transform(true, 0)
        d.nx = 0
    }

    copy(d.x[d.nx:], zeros[:limit - 18 - d.nx])

    var hsize = uint16(d.hs)

    PUTU16(d.x[limit - 18:], hsize)
    PUTU64(d.x[limit - 16:], d.len * 8)
